...
onMounted(async () => {
  try {
    // 获取作品详情
    const detailRes = await request.get(`/api/admin/work/get/${id}`, {
    ...
    // 获取 user_id
    const userRes = await request.get(`/api/admin/user/get/${work.value.u_id}`, {
      headers: { 'x-access-token': token }
    })
    ...
    // 获取主文件内容
    const mainRes = await request.get(`/api/admin/work/files/main/${id}`, {
      headers: { 'x-access-token': token }
    })
