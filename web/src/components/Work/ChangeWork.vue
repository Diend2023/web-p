...
const fetchWorkDetail = async () => {
  try {
    const res = await request.get(`/api/work/detail/${props.w_id}`, {
...
const fetchMainFiles = async () => {
  try {
    const res = await request.get(`/api/work/files/main/${props.w_id}`, {
      headers: { 'x-access-token': token }
    });
    if (res.data.code === 200) {
...
const fetchOtherFiles = async () => {
  try {
    const res = await request.get(`/api/work/files/other/${props.w_id}`, {
      headers: { 'x-access-token': token }
    });
    let filesData = [];
...
const downloadOtherFile = (file) => {
  if (file.originFileObj) {
    ...
    window.open(`${config.API_BASE_URL}/api/work/file/${props.w_id}/${file.name}`, '_blank');
  }
};
...
const handleOk = async () => {
  try {
    ...
    const res = await request.post('/api/work/set', data, {
      headers: { 'x-access-token': token }
    });
    if (res.data.code === 200) {
